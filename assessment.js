...
var getValueOfProperty = function(obj, prop) {
  for (var key in obj) {
    if (obj[key] === prop) {
    }
    return obj[prop];
...
*/
var outside = function(temp, humid, cloudy) {
    if (temp > 80 && humid > 40) {
        return "I'm all sweat";
    }
    if (temp < 40 && cloudy > 60) {
        return "I have icecicles";
    }
    if (temp > 80 && humid < 40 && cloudy < 20) {
        return "I'm literally in the desert";
    }
    if (temp > 80 || humid > 50 || cloudy > 50) {
...
var callerBack = function(fn, str) {
    return fn(str + ' back');
};
